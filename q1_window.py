...
class Config:
    ...
    window_size = 4  # The size of the window to use.
    ### YOUR CODE HERE
...
def make_windowed_data(data, start, end, window_size = 3):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i, word in enumerate(sentence):
            #Very messy
            word_features = [el for sublist in (start if j < 0 else end if j >= len(sentence) \
             else sentence[j] for j in range(i-window_size, i + window_size + 1)) for el in sublist]
            windowed_data.append((word_features, labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeds = tf.nn.embedding_lookup(emb, self.input_placeholder)
        embeddings = tf.reshape(embeds, [-1, self.config.n_window_features * self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W  = tf.get_variable("W", initializer= tf.contrib.layers.xavier_initializer(), shape=(self.config.n_window_features*self.config.embed_size, self.config.hidden_size))
        b1 = tf.get_variable("b1", initializer= tf.zeros_initializer(), shape=(self.config.hidden_size,))
        Wt  = tf.get_variable("Wt", initializer= tf.contrib.layers.xavier_initializer(), shape=(self.config.hidden_size, self.config.ht_size))
        bt = tf.get_variable("bt", initializer= tf.zeros_initializer(), shape=(self.config.ht_size,))
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            if report:
                ...
                outputs = zip(sentences, labels, predictions)
                #print([i for i in outputs])


                with open(model.config.conll_output, 'w') as f:
                    write_conll(f,  zip(sentences, labels, predictions))
                with open(model.config.eval_output, 'w') as f:
                    for sentence, label, prediction in outputs:
