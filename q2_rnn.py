...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        mask = [True for i in range(len(sentence)) if (i<len(sentence) and i<max_length)] \
               + [False for i in range(max_length - len(sentence)) if len(sentence)<max_length] 
        sentence = [word for i,word in enumerate(sentence) if i < max_length] \
        + [zero_vector for i in range(len(sentence), max_length) if len(sentence) <max_length]
        labels = [num for i,num in enumerate(labels) if i < max_length] \
        + [zero_label for i in range(len(labels), max_length) if len(labels) <max_length]
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeds, [-1, self.max_length, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_loss_op(self, preds):
        ...
        tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds, labels=self.labels_placeholder),\
        self.mask_placeholder))
        ### END YOUR CODE
        return loss
